...
`generateSkillCandidate`, `evaluateSkillCandidate`, and `SkillPromotionStore` support programmatic skill optimization. A candidate may change skill content only. Promotion recomputes eligibility from trusted Ed25519-signed training and committed frozen-holdout reports, requires exact lock lineage, minimum samples, bounded unknowns, and confidence-interval improvement, and writes durable audit receipts. Failed or unknown candidates remain diagnosable but cannot become active. A promoted skill must be resolved through the normal identity loader and receive a new manifest/lock; it cannot silently rewrite a locked benchmark.
