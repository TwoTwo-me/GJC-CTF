...
Every asynchronous producer is an owned lifecycle, not a bare promise. `createSession` returns `{ session, terminate, quiesced }` synchronously; reviewed analyzers return the same shape through `createLocalSolverAnalyzerLifecycle`; evaluation adapters, secret-sink acquisition, candidate collection, and trusted-oracle execution use `{ result, terminate, quiesced }`. A result is accepted only after quiescence. Rejection, cancellation, deadline expiry, or failed quiescence invokes bounded termination and joins quiescence again. Custom integrations must implement these contracts directly—there is no promise compatibility fallback.

Candidate campaign resume is also fail-closed. Candidate outcomes persist both `producerDigest` and `routeDigest`, and `runCtfCampaign` requires `candidateLineageFor({ challengeId })` to reproduce that exact pair before it skips a completed attempt. Missing, partial, substituted, or stale lineage reruns nothing and raises an integrity error instead of reusing the candidate.
The reviewed analyzer registry now has candidate-generation paths for all four offline corpus identities: the three active misc/reverse challenges and the Tier 2 crypto challenge `lactf-2026-crypto-not-so-lazy-trigrams`. The crypto analyzer recognizes only the pinned transform grammar, uses only `chall.py` and `ct.txt`, runs a deterministic bounded three-phase substitution search, and validates its recovered mapping by replaying the visible transform. This is analyzer coverage, not a solve claim: pwn and web still require disabled dynamic routes, all outputs remain unverified candidates, and the independently verified solve count remains zero.
