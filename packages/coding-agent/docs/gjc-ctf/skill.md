...
The LA CTF campaign pin is `3379d4a7b36680764a34e7dc817cc3c94c244764`. Its six challenges are split into two tiers: misc/reverse first, then crypto/pwn/web. Tier expansion requires independently signed verification of every eligible active-tier challenge. The hard solver-work stop is `2026-08-09T00:00:00Z` (`2026-08-09 09:00 KST`). The earlier four-candidate/two-failure campaign is invalidated because its crypto allowlist exposed an unnecessary plaintext source; those counts are audit history only. No fresh provenance-bound real-corpus campaign statistics are available, and the independently verified solve count remains zero. The harness does not solve every challenge.

## Blinded training and one-look holdout promotion

`generateSkillCandidate` and `evaluateSkillCandidate` retain diagnostic candidate evaluations, but production promotion authority belongs to `SkillOptimizationRoundStore`. A digest-sealed round commits the cohort, incumbent lineage, benchmark identities, distinct Ed25519 training/holdout registry, and finite continuation budget before selection. Exactly one signed training selection is allowed. A separate signed redacted holdout gate consumes one look; only a passed gate for the selected candidate can invoke the module-confined promotion capability. Caller-supplied verification registries are not accepted by stateful selection or finalization.

Failed or unavailable rounds can emit only bounded, cumulative-budget, unscored training continuations; absent holdout authority closes zero-look as unavailable. The active optimizer pointer cannot silently rewrite a locked benchmark skill: export/install the promoted artifact through the rooted identity loader, then create a new manifest and lock. The optimizer neither creates external permission nor supplies an oracle or calibration.
