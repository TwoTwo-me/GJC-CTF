...
The deterministic repeat runner and skill optimizer are programmatic harness surfaces, not shortcuts around benchmark authority. Five-repeat fixture output remains non-scored; optimizer promotion requires trusted Ed25519 training and holdout reports bound to the immutable parent skill and benchmark lock. Never treat task state, candidate output, archive metadata, process exit, or dashboard data as a flag or benchmark pass.
